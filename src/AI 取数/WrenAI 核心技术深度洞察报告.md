...
从旧版（GenBI Classic）改为新版（Wren AI），是整个项目对“AI时代数据分析”的**根本定位发生了彻底的转变**。

**旧版是为“人”设计的数据分析工具，而新版是为“AI代理（Agent）”设计的数据基础设施。**

---

### 🔄 转变的核心：从“人用工具”到“Agent基础设施”

| 维度 | 旧版：GenBI Classic (legacy/v1) | 新版：Wren AI (main) |
| :--- | :--- | :--- |
| **产品定位** | 一个开箱即用的**聊天式BI Web应用**。用户访问网站，通过对话获取数据。 | 一个**开放的上下文层（Open Context Layer）** 和语义引擎，供AI代理（如Claude Code, Cursor）通过CLI/SDK调用。 |
| **形态** | Docker 起的 BI 产品（Engine + AI Service + UI + Qdrant），自带 LLM→SQL 流水线 | `pip install wrenai` 一个 CLI + Rust MDL 引擎，**Agent 驱动** |
| **使用方式** | 通过浏览器使用内置的AI服务生成SQL。 | 通过 `pip install wrenai` 安装**命令行工具（CLI）**，让AI代理驱动整个流程。 |
| **核心能力** | 自带完整的AI流水线，用户提问，系统回答。 | **“治理”而非“生成”**：它本身不生成SQL，而是提供**语义层（MDL）、记忆、校验工具**，让 **你的 AI Agent（如 Claude Code、Cursor）** 在它的“护栏”内生成准确、可靠的答案。 |
| **维护状态** | **已弃用（Sunset）**，不再提供新功能和安全修复。代码被封存在 `legacy/v1` 分支。 | **活跃开发和维护**，是项目当前的主线（main分支）。 |

> 如果想要一个持续维护的、有Web界面的经典体验，官方指引是使用其商业产品 **Wren AI Commercial**，而非等待开源旧版更新。

### 💡 为什么要做这种转变？

这个转变背后，是项目团队对行业趋势的一个核心判断：**数据分析的“消费者”正在从“人”变成“AI代理”**。

1.  **适应“代理优先（Agent-First）”的未来**：未来的分析师可能不是坐在屏幕前的人，而是嵌入在Claude、Cursor或工作流中的AI代理。传统的BI工具（包括旧版WrenAI）是为人眼设计的，而代理需要的是**机器可读、结构化的“上下文”**——例如，确切的“收入”定义、正确的表关联方式等。

2.  **解决“幻觉”的根本问题**：让代理直接面对原始数据库schema，它会“猜测”业务含义，从而产生各种“幻觉”。新版WrenAI的思路是：**与其让代理去“猜”，不如用代码化的方式（MDL语义层）把“正确答案”明明白白地写出来，让代理去“读”**。你提供的MDL、知识库（`knowledge/`）等，就是这个“开放上下文层”的实体。

3.  **让治理和协作可审查**：新版WrenAI的所有关键定义（语义层、业务规则、示例查询）都是**文件**，可以被Git管理。这意味着对数据口径的任何更改都可以像代码一样经历**评审、分支、回滚**，而不是藏在某个数据库或UI里。

总结来说，这次重构是一次面向未来的“断臂求生”，让WrenAI从一个功能有限的BI应用，转变为一个支撑AI时代数据治理的基础设施组件。
