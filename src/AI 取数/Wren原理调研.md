...
| :--- | :--- | :--- | :--- |
| **链路与集成调研** | 成员 A | **“从零到查询”的完整链路是什么？**<br>CLI如何管理项目、配置、dbt集成？MCP如何让外部AI调用？ | `wren context *`<br>`wren profile *`<br>`wren context import dbt`<br>`wren serve mcp`<br>Skill: `onboarding` |
| **语义层与建模调研** | 成员 B | **数据库表如何变成Wren的语义模型？**<br>MDL是什么结构？业务规则如何注入和生效？ | `wren context build`<br>`wren memory describe`<br>`knowledge/rules/`<br>Skill: `generate-mdl`<br>Skill: `enrich-context` |
| **查询与AI交互调研** | 成员 C | **用户问题如何变成SQL？**<br>`ask`的两种模式区别？Cube查询如何简化SQL？GenBI如何生成看板？ | `wren query`<br>`wren cube *`<br>`wren ask`<br>`wren genbi *`<br>Skill: `usage`<br>Skill: `genbi` |
| **记忆与检索调研** | 成员 D | **Memory到底怎么工作的？**<br>LanceDB索引构建原理？向量检索和文本检索的区别？阈值如何影响效果？ | `wren memory *`（含 `[memory]`扩展）<br>`knowledge/sql/*.md`<br>`wren dry-plan`<br>`wren dry-run` |
...
- **输出成果**：编写一份 **“MDL 文件结构解析”** 和 **“业务规则编写最佳实践（草稿）”**。
...
| 阶段 | 时间 | 主要任务 |
| :--- | :--- | :--- |
| **第1周：独立深挖** | Day 1-5 | 各成员按分工，**不协作**，只在自己的模块内反复实验、读文档、记笔记。重点记录“意外行为”和“不明白的地方”。 |
| **第2周：整合串联** | Day 6-7 | 全员轮流进行**内部技术分享**（每人45分钟），讲解自己模块的工作原理。其他成员提问，交叉印证理解。 |
| | Day 8-10 | 根据分享中的疑问，进行**第二轮针对性补测**（特别是接口边界处）。 |
| | Day 11-12 | **集中编写**《Wren AI工作原理白皮书》初稿。 |
| | Day 13-14 | 内部评审、修改、定稿。 |

### 🎯 最终交付物
1.  **《Wren AI 工作原理白皮书》**（含各模块数据流图、命令作用域说明、关键机制解析）。
2.  **《常见问题与踩坑记录》**（基于调研中遇到的意外现象）。
3.  **一份简短的《团队后续应用建议》**（基于调研结论，指出哪些模块成熟可用、哪些有风险或限制）。

这个方案更聚焦于**“理解系统如何运转”**，而不是“如何用好它”。如果哪个角色的调研范围需要调整，我们可以再细化。
