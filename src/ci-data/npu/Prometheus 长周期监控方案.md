...
| 项目名称 | 项目类型 | 支持硬件类型 | 长时序存储方案 | 历史留存 | 看板配置核心方案 | 核心业务 / 用途 |
|----------|----------|--------------|----------------|----------|------------------|-----------------|
| 阿里内部大模型 & CosyVoice 训练集群 | 大厂内部业务集群 | NVIDIA GPU（A800/H100） | Prometheus + Thanos + 对象存储 | 6 个月 | **双看板分离**：社区 DCGM 看板 12239（查 3 天内）+ 自建聚合大盘（Thanos 数据源，仅用 5m/1h 预聚合指标） | 大模型训练/推理运维，月度算力利用率统计、偶发硬件故障回溯 |
| 滴滴 HUATUO（华佗）可观测项目 | 开源+商用异构集群基座 | 昇腾 NPU + NVIDIA GPU 异构 | Prometheus + VictoriaMetrics 集群 | 6~12 个月 | **双看板分离**：社区硬件模板（Prometheus）+ 自研聚合大盘（VM 数据源，按月统计算力利用率） | 大规模国产智算集群统一监控，异构资源长周期运营统计 |
| 昇腾 MindCluster 官方参考方案 | 国产硬件官方标准范例 | 昇腾 910B/310B NPU | Prometheus + VM 集群 + 对象存储冷归档 | 12 个月 | **官方强制双看板**：社区 NPU 看板 20592（查 3 天内）+ 自建年度资源大盘（仅用 1h 聚合指标） | 全国智算中心昇腾集群标准部署，年度硬件故障回溯、AI Core 利用率统计 |
| 浪潮云海 InCloud AIOS | 商用智算平台 | GPU + 昇腾 NPU 混合异构 | Prometheus + Thanos + MinIO | 9 个月 | **平台内置双面板**：实时运维看板（原始指标）+ 资源运营统计大屏（预聚合指标） | 政府/企业级智算中心，月度算力利用率报表、资源运营审计 |
| Databricks AI 算力集群 | 海外云厂商大规模集群 | 多品牌 GPU 服务器 | 基于 Thanos 二次开发的长时序存储底座 | 1 年以上 | **严格双看板分离**：实时面板查近 3 天细粒度数据；历史大盘仅用预聚合指标，禁止原始指标长周期查询 | 全球多区域上千台 GPU 服务器运维，万亿级指标每日写入，合规审计与历史复盘 |
| HAMi（CNCF 沙箱项目） | 开源 K8s 异构虚拟化项目 | GPU、昇腾 NPU、寒武纪 MLU 等多厂商加速器 | 官方推荐栈：Prometheus + Thanos + 对象存储 | 6 个月 | **官方明确**：实时 HAMi 看板 22043 仅短期查看；长周期统计需自建聚合面板，禁止原看板查 1 个月以上数据 | K8s vGPU 算力切片历史利用率统计、调度异常事件回溯 |
| SwanLab AI 实验平台 | 开源 AI 实验平台 | NVIDIA GPU、昇腾 NPU、寒武纪 MLU | Prometheus 热数据 + VictoriaMetrics 长时序存储 | 6 个月 | **硬件与训练联动**：实时硬件曲线（Prometheus，14 天内）+ 历史预聚合指标存入 VM，与 loss/精度曲线同屏对比 | AI 科研训练场景，硬件瓶颈与模型效果关联分析、历史实验复盘 |
| 单 Prometheus 小集群 / 实验室环境 | 个人/小团队非生产环境 | 单/少量 GPU/NPU | 仅单 Prometheus，无远端长时序存储 | 默认 15 天 | 仅 1 套社区导入看板，无分离设计 | 单机调试、短期实验监控，无长期历史回溯能力 |

> **大规模生产环境均采用双存储/双看板分离架构**
>  **长周期查询必须使用预聚合指标**（5m/1h），禁止原始指标直接查 30 天以上数据
> **VictoriaMetrics 与 Thanos 是两大主流方案**，均有大规模验证 
> 单 Prometheus 方案仅适用于非生产/实验环境，**不具备生产级长周期能力**
...
| 长周期优化 | 预聚合 5m/1h 指标，避免大时间范围查询超时 |
...
| 方案 | 托管属性 | PromQL 兼容 | 长周期性能 | 运维量 | 成本 | 华为云适配 | 推荐度 |
|------|----------|-------------|------------|--------|------|------------|--------|
| **VM 集群（CCE 自建）** | 自建 | 100% | 最优 | 中 | 低 | CCE + 云硬盘，Helm 一键部署 | ⭐⭐⭐⭐⭐ |
| **Thanos + OBS（CCE 自建）** | 自建 | 100% | 优 | 中高 | 低 | CCE + OBS 兼容 S3 | ⭐⭐⭐⭐ |
| **AOM 托管 Prometheus** | 华为云托管 | 100% | 良 | 极低 | 较高 | 远程写入，API 查询 | ⭐⭐⭐ |
| **GeminiDB(for Influx)** | 华为云托管 | 不兼容 | 中 | 极低 | 较高 | 需适配器，改造量大 | ⭐ |
| **CSS OpenSearch** | 华为云托管 | 不兼容 | 差 | 极低 | 中 | 日志为主，时序不推荐 | ⭐ |
