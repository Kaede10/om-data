...
|----------|----------|--------------|----------------|----------|------------------|-----------------|
| 阿里内部大模型 & CosyVoice 训练集群 | 大厂内部业务集群 | NVIDIA GPU（A800/H100） | Prometheus + Thanos + 对象存储 | 6 个月 | **双看板分离**：社区 DCGM 看板 12239（查 3 天内）+ 自建聚合大盘（Thanos 数据源，仅用 5m/1h 预聚合指标） | 月度算力利用率统计、偶发硬件故障回溯 |
| 滴滴 HUATUO（华佗）可观测项目 | 开源+商用异构集群基座 | 昇腾 NPU + NVIDIA GPU 异构 | Prometheus + VictoriaMetrics 集群 | 6~12 个月 | **双看板分离**：社区硬件模板（Prometheus）+ 自研聚合大盘（月统计算力利用率） | 大规模国产智算集群统一监控，异构资源长周期运营统计 |
| 昇腾 MindCluster 官方参考方案 | 国产硬件官方标准范例 | 昇腾 910B/310B NPU | Prometheus + VM 集群 + 对象存储冷归档 | 12 个月 | **官方强制双看板**：社区 NPU 看板 20592（查 3 天内）+ 自建年度资源大盘（聚合指标） | 全国智算中心昇腾集群标准部署，年度硬件故障回溯、AI Core 利用率统计 |
| 浪潮云海 InCloud AIOS | 商用智算平台 | GPU + 昇腾 NPU 混合异构 | Prometheus + Thanos + MinIO | 9 个月 | **平台内置双面板**：实时运维看板（原始指标）+ 资源运营统计大屏（预聚合指标） | 月度算力利用率报表、资源运营审计 |
| Databricks AI 算力集群 | 海外云厂商大规模集群 | 多品牌 GPU 服务器 | 基于 Thanos 二次开发的长时序存储底座 | 1 年以上 | **严格双看板分离**：实时面板查近 3 天细粒度数据；历史大盘仅用预聚合指标，禁止原始指标长周期查询 | 万亿级指标每日写入，合规审计与历史复盘 |
| HAMi（CNCF 沙箱项目） | 开源 K8s 异构虚拟化项目 | GPU、昇腾 NPU、寒武纪 MLU 等多厂商加速器 | 官方推荐栈：Prometheus + Thanos + 对象存储 | 6 个月 | **官方明确**：实时 HAMi 看板 22043 仅短期查看；长周期统计需自建聚合面板，禁止原看板查 1 个月以上数据 |算力切片历史利用率统计、调度异常事件回溯 |
| SwanLab AI 实验平台 | 开源 AI 实验平台 | NVIDIA GPU、昇腾 NPU、寒武纪 MLU | Prometheus 热数据 + VictoriaMetrics 长时序存储 | 6 个月 | **硬件与训练联动**：实时硬件曲线（14 天内）+ 历史预聚合指标存入 VM，与 loss/精度曲线同屏对比 | 硬件瓶颈与模型效果关联分析、历史实验复盘 |
...
| 方案 | 托管属性 | PromQL 兼容 | 长周期性能 | 运维量 | 成本 | 华为云适配 |
|------|----------|-------------|------------|--------|------|------------|
